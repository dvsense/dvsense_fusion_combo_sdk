...
int main(int argc, char* argv[])
{
	...
	fusionCamera->openApsExternalTrigger();
	...
	while (!stop_application) {
		if (!image_display_queue.empty()) {
			...
			std::unique_lock<std::mutex> lock_dvs(dvs_frame_mutex);
			new_frame.setTo(cv::Scalar(0, 0, 0), new_dvs_frame);
			new_dvs_frame = new_frame + new_dvs_frame;
			cv::imshow(window_name, new_dvs_frame);
