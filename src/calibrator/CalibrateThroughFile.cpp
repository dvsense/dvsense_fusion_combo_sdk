...
void CalibrateThroughFile::readParams()
{
    ...
    for (auto& [camera_id, camera_info] : camera_data.items()) {
        if (camera_info["camera_serial"] == "dvs")
        {
            ...
            dvs_params_.k = cv::Mat(3, 3, CV_64F, matrix_data.data()).clone();

            std::vector<double> rotation_data = camera_info["rotation"]["1"]["data"];
            dvs_params_.r = cv::Mat(3, 3, CV_64F, rotation_data.data()).clone();

            std::vector<double> translation_data = camera_info["translation"]["1"]["data"];
            dvs_params_.t = cv::Mat(3, 1, CV_64F, translation_data.data()).clone();
        }

        if (camera_info["camera_serial"] == "aps")
        {
            std::vector<double> matrix_data = camera_info["camera_matrix"]["data"];
            aps_params_.k = cv::Mat(3, 3, CV_64F, matrix_data.data()).clone();

            std::vector<double> rotation_data = camera_info["rotation"]["0"]["data"];
            aps_params_.r = cv::Mat(3, 3, CV_64F, rotation_data.data()).clone();

            std::vector<double> translation_data = camera_info["translation"]["0"]["data"];
            aps_params_.t = cv::Mat(3, 1, CV_64F, translation_data.data()).clone();
        }
    }
...
cv::Mat CalibrateThroughFile::getDvsToApsH(double distance)
{
	// 计算单应性矩阵 H = K2 * (R + t*nᵀ/d) * K1⁻¹
	cv::Mat H = aps_params_.k * (aps_params_.r + aps_params_.t * plane_normal_.t() / distance) * dvs_params_.k.inv();
	// 归一化矩阵
	H = H / H.at<double>(2, 2);
...
cv::Mat CalibrateThroughFile::getApsToDvsH(double distance)
{
    cv::Mat H = dvs_params_.k * (aps_params_.r + aps_params_.t * plane_normal_.t() / distance) * aps_params_.k.inv();
    H = H / H.at<double>(2, 2);
